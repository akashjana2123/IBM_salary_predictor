...
def predict():
    try:
        # Safely read values from form with defaults
        input_data = {
            ...
            'Type of ownership': request.form.get('ownership', ''),
            'Industry': request.form.get('industry', ''),
            'Sector': request.form.get('sector', ''),
            'Revenue': request.form.get('revenue', ''),
            'job_state': request.form.get('job_state', ''),
            'job_simp': request.form.get('job_simp', ''),
            'seniority': request.form.get('seniority', ''),

            'Rating': float(request.form.get('rating') or 0),
            'Founded': float(request.form.get('founded') or 0),
            ...
            'desc_len': float(request.form.get('desc_len') or 0),
            'num_comp': float(request.form.get('num_comp') or 0),

            'hourly': float(request.form.get('hourly') or 0),
            'employer_provided': float(request.form.get('employer_provided') or 0),
            'same_state': float(request.form.get('same_state') or 0),
            'python_yn': float(request.form.get('python_yn') or 0),
            'R_yn': float(request.form.get('R_yn') or 0),
            'spark': float(request.form.get('spark') or 0),
            'aws': float(request.form.get('aws') or 0),
            'excel': float(request.form.get('excel') or 0),
        }
        ...
        return render_template("index.html",
                               prediction_text=f"Predicted Average Salary: ${prediction}K")
